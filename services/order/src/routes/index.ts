...
router.get('/orders/:id/payment-url', authenticate, ctrl.getPaymentUrlForOrder);

router.get(
    '/orders/my-tickets',
    authenticate,
    [...validators.paginationRules, validate],
    ctrl.getMyTickets,
);
...
router.post(
    ...
    authenticate,
    authorize('admin'),
    [body('ticket_code').notEmpty().withMessage('Ticket code is required'), validate],
    ctrl.validateQR,
);
...
router.post(
    '/tickets/resend-email',
    authenticate,
    authorize('admin'),
    [body('ticket_id').notEmpty().withMessage('Ticket ID is required'), validate],
